...
int isGz(char* file)
{
  ...
  int l = strlen(file);
  if ( (file[l-1] == 'z') && (file[l-2] == 'g') && (file[l-3] == '.') ) 
    return 1;
  else 
    return 0;
...
FILE* fopenz(char* file, const char* code)
{
...
int nbLinesInFile(char* file)
{
  ...
  if (buff == 0)
    die("nbLinesInFile: cannot alloc buff\n");
  fp = fopen(file, "r");
  if (!fp) {
    printf("could not open set data %s\n", file);
  }
...
char* get_parameter(int argc, char** argv, const char* param)
{
  int i = 0;
...
int exist_flag(int argc, char** argv, const char* param)
{
  int i = 0;
  while ((i < argc) && (strcmp(param, argv[i])))
    i++;
...
int exist_parameter(int argc, char** argv, const char* param) {
  ...
  while (1) {  // loop over the chunks
	...
	if (s->enter_new_seq == 0) {
	  ...
	  *name = s->name;
	  ...
	  if (s->seq == 0) {
	    printf(" seqI_nextSequence_fast: Problem allocating seq, n=%d\n", s->seqlen_inc);
	    exit(0);
	  }

	  // create name
	  s->name = (char*)calloc(1000, sizeof(char));
