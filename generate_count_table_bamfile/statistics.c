 ...
 * 
 */
double FisherFrac(int k, int N, int bk, int bN) {
  int CW = N - k;
  int CM = k;
  int SW = bN - bk;
  int SM = bk;
  ...
  int s2 = CM + SM;
  int nn = SM + SW + CM + CW;
  double pv = hypergeom(ov, s1, s2, nn);
  return pv;
}
...
char* shuffleStr(char*s) 
{
  ...
  int  r, i;
  int n = strlen(s);
  snew = (char*)malloc(n * sizeof(char));
...
double maxInArray_dbl (double* a, int n)
{
	int i;
	double amax = a[0];
	for (i=1; i<n; i++) {
