...
const double ln2= 0.6931471805599452862267639829951804131269454956054688;	// log(2.0);
...
int* Construct1DCountMatrix(int* qv1, int m, int D1)
{
	...
	if (counts == 0) {
		printf("Construct1DCountMatrix: not enough memory\n");
		exit(0);
	}
...
void showCountMatrixWithFrac(int** counts, int D1, int D2)
{

	int   i, j;
	int*  sumx = 0;

	sumx = (int*)calloc(D2, sizeof(int));
	for (j=0; j<D2; j++)
		sumx[j] = 0;
	for (i=0; i<D1; i++) {
		for (j=0; j<D2; j++) {
	...
	for (j=0; j<D2; j++)
		printf("%3.1f\t", 100*counts[D1-1][j]/(float)(sumx[j]));
	printf("\n");
...
//  entropy from counts
//
float entropy_from_count_vector_char(unsigned char* P, int m)
{
...
//  entropy from counts
//
float entropy_from_count_vector_short(unsigned short* P, int m)
{
...
//  entropy from counts
//
float entropy_from_count_vector(int* P, int m)
{
...
//  entropy
//
float entropy_from_count_matrix(int* P, int n, int m)
{
		...
		for (y=0; y<D2; y++) {
			for (z=0; z<D3; z++) {
				...
				if ((counts[x][y][z] > 0) && (counts_z[z] > 0)) {
					...
					CMI += pxyz * log ( pxyz * pz / ( pxz * pyz ) ) / ln2;
	...
	for (y=0; y<D2; y++) {
		...
		for (x=0; x<D1; x++) {
			...
			if (counts [x][y] == 0) {
				...
				co = px_bar_y * log ( px_bar_y / px ) / ln2 ;
...
float UnbiasedEntropy(int* data1, int m, int D1,  int Repeats)
{
	...
	en = FindEntropy(data1, m, D1, SampleSizesPerc, N, Trials, Repeats, &std);

	return en;
}


float FindEntropy(int* qv1, int FullSample, int D1, float* SampleSizesPerc, int N, int* Trials, int Repeats, float* std)
{
	...
	for (i=0; i<N; i++) {
		SampleSizes[i]    = (int)(ceil(FullSample * SampleSizesPerc[i]));
	...
	for (r=0; r<Repeats; r++) {
		...
		for (n=0; n<N; n++) {
			...
			for (t=0; t<Trials[n]; t++) {
				...
				Ivals[n][t] = entropy(tmpqv1, tmpSampleSize, D1);
				//printf("Ivals[n][t]=%f\n", Ivals[n][t]);
			}
		...
		//
		//  END sample size loop (loop variable = n)
		//

		/*
		for (i=0; i<N; i++) {
		  printf("\t%f/%4.3f", SampleSizesInv[i], meanIvals[i]);
		}
		printf("\n");
		*/
...
float CalculateNormalizedMIbasic(int* data1,  int* data2, int m, int D1, int D2)
{
	...
	en1 = entropy(data1, m, D1);
	en2 = entropy(data2, m, D2);

	//printf("en1 = %f, en2 = %f, min=%f\n", en1, en2, min(en1,en2));

	for (i=0; i<D1; i++) {
		free(tmpCounts[i]);
	}
	free(tmpCounts);
	return mi / (min(en1,en2));
}
...
//
void CalculateAndShowCountMatrixWithFrac(int* data1,  int* data2, int m, int D1, int D2) {

	int**   tmpCounts;
	float   mymi;

	tmpCounts   = ConstructCountMatrix (data1, data2, m, D1, D2);
	showCountMatrixWithFrac(tmpCounts, D1, D2);
...
float synergy(int* M1, int* M2, int* E, int D1, int D2, int DE, int n, char* op)
{

	//
	// combine the two using a logic function
	//
	int* M12 = logicCombine(M1, M2, n, D1, D2, op);
	//int* M12 = combineQuantizedVectors(M1, M2, n, D1, D2);

	// I(M1;E)
	float mi_M1_E   = CalculateMIbasic(M1, E, n, D1, DE);

	// I(M2;E)
	float mi_M2_E   = CalculateMIbasic(M2, E, n, D2, DE);

	// I(A,B;E)
	float mi_AB_E   = CalculateMIbasic(M12, E, n, 2, DE);

	float syn       = mi_AB_E - max( mi_M1_E,  mi_M2_E );

	//if (verbose == 1)
	//printf("Details: syn = %f = %f - max( %f, %f) \n",  syn, mi_AB_E , mi_M1_E,  mi_M2_E );
...
// bounded
//
int bounded_synergy_rank_test(double score, int* M1, int D1, int* M2, int D2, int* E_q, int DE, int nborfs, int shuffle, int bound, int minshuffle, double* zscore, char* op)
{
	...
	for (j=0; j<shuffle; j++) {
		...
		mymi =  synergy(M1, M2, E_q_shu, D1, D2, DE, nborfs, op);
		...
		if ((beat >= bound) && (j >= minshuffle)) {
			break;
		}
	}

	avg = average_dbl(a_mi_shu, j_actual);
	std = stddev_dbl(a_mi_shu, j_actual);
	*zscore = (score - avg) / std;
...
float entropy(int* data, int m, int D)
{
	...
	for (i=0; i<D; i++) {
		...
		h += ( (float)bins[i] + e )/(float)m * log( ((float)bins[i] + e ) / (float)m) / ln2;
...
int CmpDecFI(const void* _a, const void* _b)
{
	const FloatAndIndex* a = (const FloatAndIndex*) _a;
	const FloatAndIndex* b = (const FloatAndIndex*) _b;
