...
int guessReadLength(char* readfile, char* format, int head)
{
  ...
  if (minl != maxl) {
    fprintf(stderr, "Warning: reads with unequal lengths detected %d and %d. Will not stop program though. \n", minl, maxl);
    //exit(1);
  }
...
void updateMismatchCountAtReadPos(char* file, int chrlen, long** poscounts, long* numnt, long* nummm, int uniquereads)
{
...
//
//  SAM format
//	adding support for extended CIGAR strings (i.e. incorporate the N operation)
//
//  returns/changes:
...
void getSimpleReadCountFromReadFile_SAM(const char* file, const int chrlen, unsigned short** readcounts, int* numreads, const int uniquereads, const int truncate, const int readlen)
{
  ...
  *numreads = 0;
  ...
  while ((line = nextLine(&li))) {
	...
	// new operation: N= skipped from reference= gap between exons
	lb= atoi(snum);
	// printf("we have N: %d\n", lb);
	idxnewread += lb;
	k= 0;
...
// will take into account all reads, not only the XT:A:U
                                  ...
                                  int* numreads, long* numnt, long* nummm, unsigned char*** ntcounts, unsigned char*** readpos, int uniquereads, int rw, int truncate, const int readlen)
{
  ...
  alignedseq = (char*)calloc(readlen * 100, sizeof(char));
  if (alignedseq == 0) die("Problem with alignedseq\n");

  myreadpos  = (char*)calloc(readlen * 100, sizeof(char));
  if (myreadpos == 0) die("Problem with myreadpos\n");

  alignedpos = (char*)calloc(readlen * 100, sizeof(char));
  if (alignedpos == 0) die("Problem with alignedpos\n");
  ...
  *numreads = 0;
  ...
  co = (unsigned char*)malloc(10000 * sizeof(unsigned char));
  if (co == 0) die("Cannot alloc co\n");
  ...
  while ((line = nextLine(&li))) {
    ...
    // get mm string
    //  m is varying. so should check for the tag MD:Z:*
    //if (idxmd == -1) {
    idxmd      = -1;
    int tmpidx = m-1;
    while (tmpidx >= 0) {
	...
	// now use hash based alignedpos and alignedseq
	// aln_nt:
	...
	*/
    ...
    for(s= aln_seq; s != NULL; s=s->hh.next) {
      ...
      if ( (s->nt != '-') && ((*readcounts)[i-1] < USHRT_MAX) && (i < chrlen)) {
	...
	if ((insertion == 1) || (mmstr[i+1] == '0') || (mmstr[i-1] == '0')) {
...
// working on extended CIGAR format
// to return:
// 	poscounts
//	numnt
//	nummm
void updateMismatchCountAtReadPos_SAM(const char* file, const int chrlen, long** poscounts, long* numnt, long* nummm, const int uniquereads, const int readlen)
{
  ...
  while (1) {	// !feof(fp))
	...
	for (i=0; i<cigrlen; i++) {
	  ...
	  if (cigr[i] == 'M') {
	    ...
	    // long gaps
	    // OE: problem here is that there are several kinds of Ns ... gaps, insert, N (but be careful, meaning not same in CIGR as in MMTR
	...
	while (i<mmstrlen) {
	  ...
	  if ((mmstr[i] == 'A') ||
	    ...
	    if (k > 0) {
	      ...
	      idxalnread += inum;
	      if (verbose == 1)
		printf("MMSTR(^): advance by %d nt, reset k\n", inum);
	      k = 0; // reset
	    ...
	    // printf("k= %d\n", k);
	    if (k > 0) { // advance for prev block
	      snum[k] = '\0';
	      inum = atoi(snum);
	      idxalnread+= inum;
	      if (verbose == 1)
		printf("MMSTR(N): advance by %d nt\n", inum);
	      k = 0;
	      // inum is usually 0 (OE 2013-07-13: but not always)
	      inum = 0; // reset
	    }
	  

	    // problem here is that we do this if k == 0, which is the case when we are in an insert and that is wrong (^N)
	    // in that case, inum from prev not set to 0

	    // MODIFIED by OE 2013-07-13 ...
	    if ((insertion == 1) || (mmstr[i+1] == '0') || (mmstr[i-1] == '0')) {
	      // N followed by 0 or N in insert => advance one more;
	      inum = 1;
	      //}
	    }
...
int formatToId(char* formattext)
{
  ...
  if (strcmp(formattext, "eland") == 0)
    format = ELAND;
  else if (strcmp(formattext, "exteland") == 0)
    format = EXTELAND;
  else if (strcmp(formattext, "mit") == 0)
    format = MIT;
  else if (strcmp(formattext, "bed") == 0)
    format = BED;
  else if (strcmp(formattext, "dnasebed") == 0)
    format = DNASEBED;
  else if (strcmp(formattext, "sam") == 0)
    format = SAM;
  else if (strcmp(formattext, "bowtiesam") == 0)
    format = BOWTIESAM;
  else if (strcmp(formattext, "bamunq") == 0) // treat bam as unique even if not (have no XT)
    format = BAMUNQ;
...
int CountReadsInChrInterval(char* file, int format, char* chr, int i1, int i2)
{
  ...
  while ((l = nextLine(&li))) {
    ...
    if (format == 3) {
      ...
      if ((m >= 12) && (strcmp(p[11], "XT:A:U") == 0) &&
	  (strcmp(p[2], chr) == 0) && (atoi(p[3]) >= i1) && (atoi(p[3]) <= i2)) {
	numreads ++;
      }
...
void getCountFromReadFile(char* file, int format, char* chr, long chrlen, int readlen, int fraglen, int uniquereads,  int uniqmap, unsigned short** counts, int* numreads, int* numclonalreads)
{
  ...
  *numclonalreads       = 0;
  ...
  while (nextRead(&ri, &r)) {
	...
	cnt_en = cnt_st+1; // we will count only 1
...
void freeRead(ReadI* ri, MappedRead *r)
{
  if (ri->formatid == EXTELAND) 
    free(r->seqname);
  ...
  if ((ri->formatid == BAM) || (ri->formatid == BAMUNQ)) {
	...
	if (ri->formatid == BAM) {
	  ...
	  if (xt[1] == 'U')
	    ...
	    r->uniqmap = 0; // R or M
	} else {  // BAMUNQ   == treat all reads as unique mappers
	  r->uniqmap = 1;
	}
	...
	if (ri->formatid == EXTELAND) {
	  ...
	  //>PC140529_0133:3:1101:7598:2236#0/1     CAATATGCAAAAGCCAAAAGATAGANNGAACAAATGCTACAAATCAGAGTT     1:0:0   chr19.fa:54466280F25AT24
	  if (p[3][0] == '-') {   // not a read
	    process = 0;
	  } else {

	    // is this a uniq mapper
	    if (p[2][0] == '1') {
	      r->uniqmap = 1;	    
	    } else {
	      r->uniqmap = 0;
	    ...
	    int idxchr = 0;
	    int idxnum = 0;
	    // start with chrom .. until .fa is reached
	    while (p[3][idx] != '.') {
	      seq[idxchr++] = p[3][idx];	      
	      idx ++;
	    }
	    seq[idxchr] = '\0';
	    r->seqname = strdup(seq); // absolutely need to release

	    idx += 4;  // skip fa:

	    // start with chrom .. until .fa is reached
	    while ((p[3][idx] != 'R') && (p[3][idx] != 'F')) {
	      snum[idxnum++] = p[3][idx];	      
	      idx ++;
	    }
	    snum[idxnum] = '\0';
	  ...
	  // DNASEBED
	} else if (ri->formatid == DNASEBED) { // && (strcmp(p[3], "U0") == 0)) {
	  // chr21   9412284 9412304 D3NH4HQ1:86:D0KK3ACXX:2:1106:14620:150981       1       +
	  ...
	  if (p[5][0] == '-')
	    ...
	    r->st = 1;
	  r->uniqmap = 1;  // assumed by default
                              ...
                              int* numreads, long* numnt, long* nummm, int verbose)
{
  ...
  *numreads = 0;
  ...
  while ((line = nextLine(&li))) {
    ...
    // if (idxalnread != strlen(alignedseq))
    if (idxalnread != alignedseqlen) {
      printf("MMSTR: Houston, we have a pb in CountMismatchesInSAMFile(), idxalnread= %d, len= %d, pos= %d, mmstr= %s, cigar= %s, file= %s\n", idxalnread, alignedseqlen, pos, mmstr, cigr, file);
    }
