...
int main(int argc, char ** argv) {
 ...
 T_t = (float*) calloc (seq_count, sizeof(float)) ;
 int nhits=0 ;
 ...
 float ave = average (T_t, seq_count) ;
 float std = stddev (T_t, seq_count) ;
 printf("Average match = %f; std = %f.\nThreshold set to: %f", ave, std, ave+std) ; fflush(stdout) ;
