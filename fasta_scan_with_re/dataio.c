...
float* read_expfile (char *expfile, char ***names, int *count){
  int i ;
  int mynmax = 100000 ;
  int   hashret =0 ;
  char* s ;
  ...
  fflush(stdout) ;
  *names   = (char**) malloc (mynmax * sizeof(char*)) ;
  float *E = (float*) malloc (mynmax * sizeof(float)) ;
  *count = 0 ;
...
void printFloatTable(int m, int n, float** data){
  int i, j ;
  for (j=0 ; j<n ; j++){
  	printf("\t%d", j) ;
  }
...
char* nextSequence(FILE* fp, char** name, int* size, int* nextSequence_started, int* nextSequence_ended, char* nextSequence_currentLine)
{
  ...
  if (nextSequence_currentLine[0] == '>') {
    ...
    *name = strdup(nextSequence_currentLine + 1);
    ...
    // create a new line
    // changed from 100000 (not required)
    seq = (char*)calloc(maxlen, sizeof(char) );
	...
	*nextSequence_ended  = 1;
	...
	li = strlen(seq);
	le = strlen(nextSequence_currentLine);
	if (li + le > maxlen) {
	  die("Sorry, this software cannot handle sequences larger than 10kb. Please modify dataio.c and re-compile or contact us.\n");
	}
	strncat(seq, nextSequence_currentLine, le);
	seq[li+le] = '\0';
      }
